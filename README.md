...
```bash
test -d busco_downloads/lineages/arthropoda_odb10   && echo "[OK] BUSCO arthropoda_odb10 is available."
```
...
```bash
POLOCO_OVERRIDE_CONFIG=configs/poloco_smoke_config.sh bash scripts/00_check_inputs.sh full
```
...
The complete public input occupied approximately 100 GB in the tested download. Consider the additional space required for trimmed FASTQs, alignment intermediates, filtered BAMs, the mpileup, the sync file, and QC outputs before starting the full workflow.
...
bash scripts/00_check_inputs.sh full   2>&1 | tee logs/ena_full_preflight.log
```
...
```bash
sed -E 's#.*/##; s/\.filtered\.bam$//'   PoPoolation2/metadata/bamlist_clean.txt   > PoPoolation2/metadata/sample_order.txt
```
...
Step 02 skips an existing complete assembly, but stops when the assembly directory exists without the expected final contigs. Step 06 skips existing mpileup and sync files. The current Step 04 processes samples sequentially; inspect existing BAM outputs carefully before restarting it.
