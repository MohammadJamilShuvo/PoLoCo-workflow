# 🧬 PoLoCo Workflow

**Reproducible workflow for pooled draft genome assembly and Pool-seq allele-frequency analysis of non-model invertebrates**

![workflow](https://github.com/user-attachments/assets/20c36866-0dfd-4626-a3d5-9b92721b1bf1)

---

## 📖 Overview

**PoLoCo** (Pooled Low-Coverage) is an open-source workflow for projects where population genomic analyses need to be performed on small, non-model invertebrates for which high-quality individual reference genomes are difficult to obtain.

The workflow supports:

- **Draft genome assembly** from a pooled assembly library, for example ethanol-preserved specimens.
- **Automatic reference preparation** from the draft assembly for downstream mapping.
- **Assembly validation** against an optional published or external reference genome.
- **Read preprocessing, mapping, and BAM filtering** for pooled population libraries.
- **Pool-seq sync generation and allele-frequency matrix construction** using PoPoolation2 and configurable downstream filtering.
- **QC summaries and visualization** for read quality, mapping, coverage, and final allele-frequency outputs.

PoLoCo produces filtered SNP tables and allele-frequency matrices that can be used for downstream population-genomic or landscape-genomic analyses. The workflow itself focuses on reproducible data processing up to the allele-frequency matrix stage.

The workflow was developed for the *Entomobrya nivalis* case study described in the manuscript:

> **PoLoCo: A reproducible pooled low-coverage workflow for draft genome assembly and allele frequency analysis from ethanol-preserved small non-model invertebrates**  
> Mohammad Jamil Shuvo, Gernot Segelbacher, Julia C. Geue, [link pending journal publication].

---

## Repository structure

The main folders are:

```text
PoLoCo-workflow/
├── README.md
├── configs/
│   ├── install_poloco_conda_envs.sh
│   ├── poloco_config.sh
│   ├── poloco_smoke_config.sh
│   ├── poloco_user_config.example.sh
│   └── slurm_config.example.sh
├── smoke_test/
│   └── data/
│       ├── assembly/
│       │   ├── assembly_pool_R1.fastq.gz
│       │   └── assembly_pool_R2.fastq.gz
│       └── pools/
│           ├── smoke_pool_01_R1.fastq.gz
│           ├── smoke_pool_01_R2.fastq.gz
│           ├── smoke_pool_02_R1.fastq.gz
│           ├── smoke_pool_02_R2.fastq.gz
│           ├── smoke_pool_03_R1.fastq.gz
│           └── smoke_pool_03_R2.fastq.gz
├── ena_example/
│   ├── poloco_ena_case_study_manifest.tsv
│   └── download_poloco_ena_case_study_reads.py
├── hpc/
│   ├── run_poloco_slurm_step.sh
│   └── submit_poloco_slurm.sh
├── scripts/
│   ├── 00_check_inputs.sh
│   ├── 01_preprocessing_qc.sh
│   ├── 02_assembly.sh
│   ├── 03_validation.sh
│   ├── 04_mapping_dedup_filter.sh
│   ├── 05_coverage_depth.sh
│   ├── 06_poolseq_pipeline.sh
│   ├── 07_qc_visualization.sh
│   ├── 08_neutral_connectivity.sh
│   └── run_poloco_pipeline.sh
├── connectivity_example/
│   └── plot_coordinates.csv
└── qc_scripts/
    ├── fastp_qc.py
    ├── alignment_qc.py
    ├── poolseq_af_qc.py
    ├── poolseq_summary_stats_qc.py
    └── final_qc_master.py
```

The repository includes a fixed lightweight smoke-test dataset (`smoke_test/data/`) for rapid end-to-end workflow validation and an ENA case-study example (`ena_example/`) for reproducing the published analysis. All user-facing instructions are provided in this `README.md`.

---

<!-- POLOCO_COLLAPSE_END -->
</details>
<details>
<summary><strong>⚙️ Installation</strong></summary>
<!-- POLOCO_COLLAPSE_START -->

### 1. Clone the repository

```bash
git clone git@github.com:MohammadJamilShuvo/PoLoCo-workflow.git
cd PoLoCo-workflow
```

HTTPS can also be used:

```bash
git clone https://github.com/MohammadJamilShuvo/PoLoCo-workflow.git
cd PoLoCo-workflow
```

### 2. Install the required conda environments

PoLoCo uses modular conda environments internally because the workflow combines tools with different dependency requirements. To keep installation simple, all required environments are created with one installer script:

```bash
bash configs/install_poloco_conda_envs.sh
```

This creates missing environments and updates existing environments:

```text
poloco_qc_mapping
poloco_poolseq
poloco_assembly
poloco_connectivity
```

These environments are used automatically by the workflow scripts.

| Environment | Used for |
|---|---|
| `poloco_qc_mapping` | read preprocessing, FastQC/MultiQC, mapping, BAM filtering, coverage |
| `poloco_poolseq` | mpileup/sync generation, Pool-seq filtering, allele-frequency matrix, QC summaries |
| `poloco_assembly` | draft assembly and assembly validation |
| `poloco_connectivity` | neutral population structure, spatial mapping, and isolation-by-distance analysis |

The installer fixes the validated Pool-seq toolchain at samtools 1.23.1, htslib 1.23.1, and bcftools 1.23.1.

Verify the three environments after installation:

```bash
conda run -n poloco_qc_mapping fastp --version
conda run -n poloco_poolseq bash -lc 'command -v mpileup2sync.pl'
conda run -n poloco_poolseq samtools --version
conda run -n poloco_assembly busco --version
conda run -n poloco_connectivity Rscript --version
```

The Pool-seq samtools check should begin with:

```text
samtools 1.23.1
Using htslib 1.23.1
```

On an HPC system, install the environments in project or workspace storage rather than a quota-limited home directory. For example:

```bash
mkdir -p /path/to/workspace/conda/envs
mkdir -p /path/to/workspace/conda/pkgs

conda config --prepend envs_dirs /path/to/workspace/conda/envs
conda config --prepend pkgs_dirs /path/to/workspace/conda/pkgs
```

If Conda is provided as a module, load it before running the installer. A tested bwUniCluster 3 setup was:

```bash
module load devel/miniforge
source "$(conda info --base)/etc/profile.d/conda.sh"
export CONDA_SOLVER=libmamba

bash configs/install_poloco_conda_envs.sh
```

Module and partition names are cluster-specific. Run a large installation in an interactive compute allocation or a batch job rather than on a busy login node.

For a full assembly assessment, download the BUSCO Arthropoda lineage once from the repository root:

```bash
conda run -n poloco_assembly busco --download arthropoda_odb10
```

Verify it:

```bash
test -d busco_downloads/lineages/arthropoda_odb10 \
  && echo "[OK] BUSCO arthropoda_odb10 is available."
```

The fixed smoke test sets `RUN_BUSCO="no"` and therefore does not require this lineage.

To preserve the exact installed software versions used for a reproducible run:

```bash
mkdir -p reproducibility_records

for ENV in poloco_qc_mapping poloco_poolseq poloco_assembly poloco_connectivity
do
    conda env export -n "$ENV" \
      > "reproducibility_records/${ENV}.yml"

    conda list --explicit -n "$ENV" \
      > "reproducibility_records/${ENV}_explicit.txt"
done
```

You normally do not need to activate environments manually when running the full workflow through `scripts/run_poloco_pipeline.sh`. For testing a single environment manually, use for example:

```bash
conda activate poloco_qc_mapping
```

---

<!-- POLOCO_COLLAPSE_END -->
</details>
<details>
<summary><strong>🧩 Configure the workflow</strong></summary>
<!-- POLOCO_COLLAPSE_START -->
## 🧩 Configure the workflow

PoLoCo uses one permanent main configuration and optional dataset-specific override files:

```text
configs/poloco_config.sh
configs/poloco_smoke_config.sh
configs/poloco_user_config.example.sh
configs/slurm_config.example.sh
```

`configs/poloco_config.sh` contains the default *Entomobrya nivalis* case-study settings. All workflow scripts continue to source this same main file.

`configs/poloco_smoke_config.sh` contains only the settings that differ for the fixed lightweight smoke test. It is loaded as an override, so the case-study configuration does not need to be edited or restored.

`configs/poloco_user_config.example.sh` is the template for a user's own dataset or a reduced ENA selection. It creates an independent project root and should be copied before editing.

`configs/slurm_config.example.sh` contains site-specific SLURM resources and module settings. Copy it to `configs/slurm_config.sh` before an HPC run. The local `configs/slurm_config.sh` file is ignored by Git.

### Which configuration should be used?

| Run type | Configuration approach | Command |
|---|---|---|
| Fixed smoke test | use the supplied smoke override | `POLOCO_OVERRIDE_CONFIG=configs/poloco_smoke_config.sh bash scripts/run_poloco_pipeline.sh --mode full` |
| Full ENA case study | use the default main configuration | `bash scripts/run_poloco_pipeline.sh --mode full` |
| Reduced ENA selection | copy the user-config example and adjust project paths and thresholds | `POLOCO_OVERRIDE_CONFIG=configs/my_project_config.sh bash scripts/run_poloco_pipeline.sh --mode full` |
| User's own dataset | copy the user-config example and edit it | `POLOCO_OVERRIDE_CONFIG=configs/my_project_config.sh bash scripts/run_poloco_pipeline.sh --mode full` |

### Case-study defaults

The default `configs/poloco_config.sh` contains:

```bash
PROJECT_PREFIX="envilis"
RAW_DIR="01_raw_reads"
MINCOV=4
MINPOP_MODE="absolute"
MINPOP_ABS=55
MINPOP_PROP=0.67
MAF=0.05
THIN_DIST=200
MAPQ=30
BASEQ=20
RUN_BUSCO="yes"
RUN_CONNECTIVITY_ANALYSIS="yes"
COORDINATES_FILE="connectivity_example/plot_coordinates.csv"
MANTEL_PERMUTATIONS=9999
```

These values are intended for the complete case study with 82 Pool-seq libraries. The supplied coordinate file contains the matching 82 projected plot coordinates and is used automatically in Step 08.

### Smoke-test configuration

The supplied `configs/poloco_smoke_config.sh` already contains:

```bash
PROJECT_PREFIX="poloco_smoke"
RAW_DIR="smoke_test/data"
MINCOV=2
MINPOP_MODE="absolute"
MINPOP_ABS=2
MAF=0.01
THIN_DIST=50
MIN_CONTIG_LEN=500
RUN_BUSCO="no"
RUN_CONNECTIVITY_ANALYSIS="no"
```

Users should not edit the main case-study config to run the smoke test.

### User's own dataset

Create a separate override from the user-project template:

```bash
cp configs/poloco_user_config.example.sh configs/my_project_config.sh
```

Do not copy the smoke-test configuration for a real project. The smoke file intentionally redirects all outputs to `smoke_test/output/`, uses relaxed thresholds, and disables BUSCO.

Then edit `configs/my_project_config.sh`. At minimum, check:

| Setting | What to adjust |
|---|---|
| `PROJECT_PREFIX` | short project/species label |
| `RAW_DIR`, `ASSEMBLY_RAW_DIR`, `POOL_RAW_DIR` | locations of the input FASTQs |
| `ASSEMBLY_R1`, `ASSEMBLY_R2` | assembly read filenames |
| output directories | use a separate project output root |
| `READ1_SUFFIX`, `READ2_SUFFIX` | change only if paired-read suffixes differ |
| `REF_FASTA` | required for `poolseq-only` mode |
| `PUBLISHED_REF` | optional external genome for validation |
| `MINCOV` | minimum coverage per pool |
| `MINPOP_MODE`, `MINPOP_ABS`, `MINPOP_PROP` | minimum informative pools |
| `MAF`, `THIN_DIST` | SNP filtering |
| `RUN_BUSCO` | `yes` for full assembly assessment, `no` for a quick functional test |
| `RUN_CONNECTIVITY_ANALYSIS` | set to `yes` to run PCA and isolation-by-distance analysis |
| `COORDINATES_FILE` | CSV with `pool_id`, `x`, and `y`; x/y must be projected coordinates in metres |
| `MANTEL_PERMUTATIONS` | number of permutations for the Mantel test |
| thread settings | adjust to the available computer or HPC allocation |

For a dataset with a different number of pools, do not retain the case-study value `MINPOP_ABS=55`. Proportional filtering is often easier:

```bash
MINPOP_MODE="proportion"
MINPOP_PROP=0.67
```

---

<!-- POLOCO_COLLAPSE_END -->
</details>
<details>
<summary><strong>📥 Input data</strong></summary>
<!-- POLOCO_COLLAPSE_START -->
## 📥 Input data

PoLoCo requires **paired-end Illumina FASTQ files**. Inputs can be provided in four ways:

1. run the fixed lightweight smoke test included in the repository,
2. use local FASTQ files,
3. download the public PoLoCo case-study reads from ENA,
4. adapt the ENA manifest to another public ENA dataset.

### Expected input folder structure

PoLoCo expects assembly reads and pooled population reads to be separated:

```text
01_raw_reads/
├── assembly/
│   ├── assembly_pool_R1.fastq.gz
│   └── assembly_pool_R2.fastq.gz
└── pools/
    ├── pool_001_R1.fastq.gz
    ├── pool_001_R2.fastq.gz
    ├── pool_002_R1.fastq.gz
    ├── pool_002_R2.fastq.gz
    └── ...
```

Use:

```text
01_raw_reads/assembly/
```

for the paired-end reads used to build the draft genome.

Use:

```text
01_raw_reads/pools/
```

for the paired-end reads used for population-level Pool-seq mapping and allele-frequency estimation.

Input files should be gzip-compressed paired-end FASTQ files, preferably named with:

```text
_R1.fastq.gz
_R2.fastq.gz
```

If your files use another naming pattern, update `READ1_SUFFIX` and `READ2_SUFFIX` in the selected project-specific override. Avoid editing the permanent case-study defaults for a separate user project.

### Option 1: Run the fixed lightweight smoke test

The repository includes a small, fixed paired-end test dataset under:

```text
smoke_test/data/
├── assembly/
│   ├── assembly_pool_R1.fastq.gz
│   └── assembly_pool_R2.fastq.gz
└── pools/
    ├── smoke_pool_01_R1.fastq.gz
    ├── smoke_pool_01_R2.fastq.gz
    ├── smoke_pool_02_R1.fastq.gz
    ├── smoke_pool_02_R2.fastq.gz
    ├── smoke_pool_03_R1.fastq.gz
    └── smoke_pool_03_R2.fastq.gz
```

These files are permanently stored in the repository; users do not generate them. They are intended only to check that Steps 00–07 connect and execute correctly. They do not reproduce the manuscript assembly statistics or biological results.

The smoke test uses the supplied override configuration. No manual editing of `configs/poloco_config.sh` is required.

Remove an earlier smoke-test output, if present:

```bash
rm -rf smoke_test/output
rm -f smoke_test_run.log
```

Run the input and configuration preflight:

```bash
POLOCO_OVERRIDE_CONFIG=configs/poloco_smoke_config.sh \
bash scripts/00_check_inputs.sh full
```

The preflight should detect three Pool-seq libraries.

Run the complete smoke test:

```bash
POLOCO_OVERRIDE_CONFIG=configs/poloco_smoke_config.sh \
bash scripts/run_poloco_pipeline.sh --mode full \
  2>&1 | tee smoke_test_run.log
```

A successful functional test ends with:

```text
[OK] PoLoCo workflow completed: full
```

The validated smoke test produces the following main output folders under `smoke_test/output/`:

```text
02_trimmed_reads/
02_fastqc_reports/
03_alignments/
04_bam_filtered/
05_coverage/
07_plots/
08_assembly/
09_validation/
PoPoolation2/
ref/
```

The fixed smoke dataset is expected to retain eight polymorphic sites after the current minimum-population, MAF, and distance-thinning filters. The final matrix should therefore contain eight rows:

```text
smoke_test/output/PoPoolation2/results/geno_AF_matrix_LD_MAF001.csv
```

Check the principal outputs with:

```bash
find smoke_test/output -maxdepth 3 -type f | sort

cat smoke_test/output/PoPoolation2/results/poloco_smoke_filtering_summary.tsv

wc -l smoke_test/output/PoPoolation2/results/geno_AF_matrix_LD_MAF001.csv

ls -lh smoke_test/output/07_plots/poloco_master_qc.xlsx
```

The expected filtering summary is:

```text
Step                      Retained_Sites
sync_rows_total           119970
polymorphic_rows          8
after_minpop              8
after_maf                 8
after_distance_thinning   8
```

Small numerical differences may occur with substantially different software versions, but the workflow should complete and retain a non-empty SNP table and allele-frequency matrix.

The default case-study configuration remains unchanged after the smoke test.

The following messages can be non-fatal during the fixed smoke test:

- no `smoke_test/output/ref/ncbi_reference.fa`, so comparative QUAST, FastANI, and MUMmer validation is skipped;
- BWA may report too few read pairs for one orientation because the fixed test dataset is very small;
- Picard may print command-syntax transition, optical-duplicate, or Java native-access warnings;
- WSL may print locale warnings.

Judge success from the final workflow message, the required output files, and the expected filtering summary rather than from the presence of informational warnings.

### Option 2: Use local FASTQ files

Create the input folders:

```bash
mkdir -p 01_raw_reads/assembly
mkdir -p 01_raw_reads/pools
```

Copy the assembly-pool reads:

```bash
cp /path/to/assembly_pool_R1.fastq.gz 01_raw_reads/assembly/
cp /path/to/assembly_pool_R2.fastq.gz 01_raw_reads/assembly/
```

Copy the pooled population reads:

```bash
cp /path/to/pool_*_R1.fastq.gz 01_raw_reads/pools/
cp /path/to/pool_*_R2.fastq.gz 01_raw_reads/pools/
```

Then create a project-specific override:

```bash
cp configs/poloco_user_config.example.sh configs/my_project_config.sh
```

Update its input/output paths, filenames, thresholds, and resources, and run PoLoCo with `POLOCO_OVERRIDE_CONFIG`. This keeps the supplied case-study and smoke-test configurations unchanged.

### Option 3: Download the PoLoCo case-study reads from ENA

The manuscript case study uses ethanol-preserved *Entomobrya nivalis* pooled libraries deposited under ENA BioProject:

```text
PRJEB111482
```

The repository includes:

```text
ena_example/
├── poloco_ena_case_study_manifest.tsv
└── download_poloco_ena_case_study_reads.py
```

The manifest separates the single assembly library from the 82 retained Pool-seq libraries.

First, run a dry run to see what would be downloaded:

```bash
conda run -n poloco_poolseq python ena_example/download_poloco_ena_case_study_reads.py \
  --manifest ena_example/poloco_ena_case_study_manifest.tsv \
  --outdir 01_raw_reads \
  --dry-run
```

To download the assembly library and three Pool-seq libraries as a reduced **real-data selection** (still a large download and not the lightweight smoke test):

```bash
conda run -n poloco_poolseq python ena_example/download_poloco_ena_case_study_reads.py \
  --manifest ena_example/poloco_ena_case_study_manifest.tsv \
  --outdir 01_raw_reads \
  --max-pools 3
```

To download the full case-study dataset:

```bash
conda run -n poloco_poolseq python ena_example/download_poloco_ena_case_study_reads.py \
  --manifest ena_example/poloco_ena_case_study_manifest.tsv \
  --outdir 01_raw_reads
```

The downloader places files into:

```text
01_raw_reads/assembly/
01_raw_reads/pools/
```

and verifies MD5 checksums when available.

The downloader uses temporary `.part` files, retries interrupted transfers, and can resume a partial file when the server supports byte-range requests. An existing completed FASTQ is skipped only after its MD5 checksum is verified when an expected checksum is available.

For the complete 82-pool case study, verify the files before running the workflow:

```bash
printf "Assembly R1: "
find 01_raw_reads/assembly -maxdepth 1 -name '*_R1.fastq.gz' | wc -l

printf "Assembly R2: "
find 01_raw_reads/assembly -maxdepth 1 -name '*_R2.fastq.gz' | wc -l

printf "Pool R1: "
find 01_raw_reads/pools -maxdepth 1 -name '*_R1.fastq.gz' | wc -l

printf "Pool R2: "
find 01_raw_reads/pools -maxdepth 1 -name '*_R2.fastq.gz' | wc -l

printf "Incomplete .part files: "
find 01_raw_reads -name '*.part' | wc -l

du -sh 01_raw_reads
```

Expected counts:

```text
Assembly R1: 1
Assembly R2: 1
Pool R1: 82
Pool R2: 82
Incomplete .part files: 0
```

The complete public input occupied approximately 100 GB in the tested download.

For the complete 82-pool case study, reserve at least **1 TB of working storage**. In the complete validation run, `PoPoolation2/results/` alone occupied approximately 530 GB, including an approximately 202 GB mpileup, 307 GB sync file, and 21 GB filtered SNP sync file. Additional space is required for raw reads, trimmed reads, alignments, assembly outputs, coverage files, and QC results.

Run the full-data preflight with the default configuration:

```bash
unset POLOCO_OVERRIDE_CONFIG

bash scripts/00_check_inputs.sh full \
  2>&1 | tee logs/ena_full_preflight.log
```

The critical expected values are:

```text
Project prefix: envilis
Pool-seq paired-end libraries detected: 82
MINCOV=4
MINPOP_MODE=absolute
MINPOP_ABS=55
MINPOP_PROP=0.67
MAF=0.05
THIN_DIST=200
MAPQ=30; BASEQ=20
Input and configuration check completed
```

For the complete 82-pool case study, the default `configs/poloco_config.sh` can be used. For a reduced ENA selection, create a separate override from `configs/poloco_user_config.example.sh` and reduce `MINPOP_ABS` or use proportional filtering before running the workflow.

### Option 4: Adapt the ENA example to another public ENA dataset

Users with their own public ENA data can adapt:

```text
ena_example/poloco_ena_case_study_manifest.tsv
```

Use the same columns as the supplied manifest. The essential fields used by the downloader are:

```text
sample_id
study_accession
library_role
library_name
submitted_read1
submitted_read2
read1_md5
read2_md5
standard_read1
standard_read2
```

Use `library_role=assembly` for the library used to build the draft reference genome.

Use `library_role=poolseq` for pooled population libraries used for mapping and allele-frequency estimation.

After editing the manifest, run:

```bash
conda run -n poloco_poolseq python ena_example/download_poloco_ena_case_study_reads.py \
  --manifest ena_example/poloco_ena_case_study_manifest.tsv \
  --outdir 01_raw_reads
```

Then create or update a project-specific override configuration for that dataset and run PoLoCo with `POLOCO_OVERRIDE_CONFIG`.


### Coordinates for neutral population structure and connectivity

Step 08 uses the final allele-frequency matrix and the exact sample order stored in:

```text
PoPoolation2/metadata/bamlist_clean.txt
```

The complete *Entomobrya nivalis* case study uses the supplied file:

```text
connectivity_example/plot_coordinates.csv
```

The coordinate file contains:

```text
pool_id,x,y
```

`pool_id` must match the sample names represented in the BAM list. The supplied case-study file uses the numeric pool identifiers corresponding to names such as `ENIV_pool_2501`. The script resolves this prefix automatically.

Coordinates must be projected x/y coordinates in metres. Longitude and latitude in decimal degrees should be converted to an appropriate projected coordinate reference system before running Step 08.

For another dataset, place a coordinate CSV in the project directory and set the following in the project-specific configuration:

```bash
RUN_CONNECTIVITY_ANALYSIS="yes"
COORDINATES_FILE="projects/my_project/plot_coordinates.csv"
MANTEL_PERMUTATIONS=9999
```

Set `RUN_CONNECTIVITY_ANALYSIS="no"` when coordinates are unavailable. The fixed smoke test does this automatically.

---

<!-- POLOCO_COLLAPSE_END -->
</details>
<details>
<summary><strong>📂 Workflow steps</strong></summary>
<!-- POLOCO_COLLAPSE_START -->
## 📂 Workflow steps

The simplified PoLoCo core workflow is organized as follows. The assembly step also prepares the draft assembly as the mapping reference, so there is no separate reference-preparation step.

| Step | Script | Main tools | Environment | Main outputs |
|---|---|---|---|---|
| 0 | `scripts/00_check_inputs.sh` | Bash | active shell | checks config, folders, read pairs, required inputs |
| 1 | `scripts/01_preprocessing_qc.sh` | fastp, FastQC, MultiQC | `poloco_qc_mapping` | trimmed reads, fastp/FastQC/MultiQC summaries |
| 2 | `scripts/02_assembly.sh` | MEGAHIT, BUSCO, BWA, samtools | `poloco_assembly` | draft assembly, `ref/poloco_draft.fa`, reference indexes |
| 3 | `scripts/03_validation.sh` | QUAST, FastANI, MUMmer4 | `poloco_assembly` | assembly metrics and optional reference comparison |
| 4 | `scripts/04_mapping_dedup_filter.sh` | BWA, samtools, Picard | `poloco_qc_mapping` | mapped, deduplicated, filtered BAM files |
| 5 | `scripts/05_coverage_depth.sh` | samtools | `poloco_qc_mapping` | per-pool coverage summaries |
| 6 | `scripts/06_poolseq_pipeline.sh` | samtools, PoPoolation2, Bash/awk | `poloco_poolseq` | BAM list, mpileup, sync file, filtered SNP table, allele-frequency matrix |
| 7 | `scripts/07_qc_visualization.sh` | Python QC scripts | `poloco_poolseq` | QC plots and integrated QC summaries |
| 8 | `scripts/08_neutral_connectivity.sh` | R, PCA, Mantel test | `poloco_connectivity` | neutral-structure figures, isolation-by-distance results, tables, Excel workbook |

QC helper scripts are in `qc_scripts/`:

```text
fastp_qc.py
alignment_qc.py
poolseq_af_qc.py
poolseq_summary_stats_qc.py
final_qc_master.py
```

---

<!-- POLOCO_COLLAPSE_END -->
</details>
<details>
<summary><strong>🚀 Running the workflow</strong></summary>
<!-- POLOCO_COLLAPSE_START -->
## 🚀 Running the workflow

Before running PoLoCo:

1. install conda environments,
2. prepare or download input FASTQ files,
3. select the appropriate configuration or override,
4. choose the workflow mode.

### Full workflow

Use this when you want PoLoCo to preprocess reads, assemble a draft genome, prepare the reference, validate the assembly, map Pool-seq libraries, and create the allele-frequency matrix.

For the complete ENA case study, use the default configuration:

```bash
bash scripts/run_poloco_pipeline.sh --mode full
```

For the fixed smoke test, use:

```bash
POLOCO_OVERRIDE_CONFIG=configs/poloco_smoke_config.sh \
bash scripts/run_poloco_pipeline.sh --mode full
```

For a user's own project, use a project-specific override:

```bash
POLOCO_OVERRIDE_CONFIG=configs/my_project_config.sh \
bash scripts/run_poloco_pipeline.sh --mode full
```

The full workflow runs:

```text
00_check_inputs
01_preprocessing_qc
02_assembly
03_validation
04_mapping_dedup_filter
05_coverage_depth
06_poolseq_pipeline
07_qc_visualization
08_neutral_connectivity
```

### Pool-seq-only workflow

Use this when you already have a reference genome and only want to run preprocessing, mapping, coverage, Pool-seq filtering, and QC summaries:

```bash
bash scripts/run_poloco_pipeline.sh --mode poolseq-only
```

Before using this mode, set `REF_FASTA` in the selected project-specific override to the reference genome you want to use. The reference should already be available; PoLoCo creates BWA and FASTA indexes if they are missing.

### Assembly-only workflow

Use this when you only want to assemble and validate a draft genome:

```bash
bash scripts/run_poloco_pipeline.sh --mode assembly-only
```

### Run one step

For debugging or rerunning a specific part of the default case-study workflow:

```bash
bash scripts/run_poloco_pipeline.sh --step 06
```

For the smoke test or another override configuration:

```bash
POLOCO_OVERRIDE_CONFIG=configs/poloco_smoke_config.sh \
bash scripts/run_poloco_pipeline.sh --step 06
```

The step number corresponds to the workflow table above.

### SLURM/HPC use

The master runner executes all steps sequentially and should be run with `bash` on a workstation or inside an interactive HPC allocation. Do not submit the master runner itself with `sbatch`, because the resource directives inside child scripts are ignored when they are called with `bash`.

For reproducible SLURM batch execution, the repository provides:

```text
hpc/run_poloco_slurm_step.sh
hpc/submit_poloco_slurm.sh
configs/slurm_config.example.sh
```

The launcher submits the numbered workflow scripts as separate jobs connected by `afterok` dependencies. It also loads the cluster-specific Conda setup inside every compute job, preserves the selected override configuration, checks that configured thread counts do not exceed the allocated CPUs, and records submitted job IDs.

Create a local site configuration:

```bash
cp configs/slurm_config.example.sh configs/slurm_config.sh
```

Edit `configs/slurm_config.sh`. A tested bwUniCluster 3 example is:

```bash
SLURM_PARTITION="cpu_il"
MODULE_SETUP='module load devel/miniforge'
SLURM_THREADS_PER_CORE="1"
```

Module, account, QOS, partition, memory, and runtime settings are cluster-specific. `configs/slurm_config.sh` is ignored by Git.

Check syntax:

```bash
bash -n hpc/run_poloco_slurm_step.sh
bash -n hpc/submit_poloco_slurm.sh
```

Submit the complete ENA case study with the permanent default configuration:

```bash
unset POLOCO_OVERRIDE_CONFIG
bash hpc/submit_poloco_slurm.sh --mode full
```

Submit the fixed smoke test:

```bash
bash hpc/submit_poloco_slurm.sh \
  --mode full \
  --config configs/poloco_smoke_config.sh
```

Submit a user's own full project:

```bash
bash hpc/submit_poloco_slurm.sh \
  --mode full \
  --config configs/my_project_config.sh
```

The launcher writes a job table to:

```text
reproducibility_records/poloco_slurm_jobs_<timestamp>.tsv
```

Monitor the latest submitted workflow:

```bash
JOB_FILE=$(ls -1t reproducibility_records/poloco_slurm_jobs_*.tsv | head -n 1)

JOBS=$(tail -n +2 "$JOB_FILE" | cut -f2 | paste -sd, -)

squeue -j "$JOBS" \
  -o "%.18i %.12P %.25j %.2t %.10M %.30R"

sacct -X -j "$JOBS" \
  --format=JobID,JobName,State,Elapsed,MaxRSS,ExitCode
```

Typical states:

- `PD (Priority)`: waiting for scheduling priority;
- `PD (Dependency)`: correctly waiting for the previous step;
- `R`: running;
- `COMPLETED` with exit code `0:0`: successful;
- `FAILED`, `OUT_OF_MEMORY`, `TIMEOUT`, or a non-zero exit code: inspect that step before resubmitting.

Closing the terminal does not stop submitted batch jobs.

For users who prefer to construct the dependency chain manually, the original approach remains valid after ensuring that Conda is available inside every job:

```bash
mkdir -p logs

J0=$(sbatch --parsable scripts/00_check_inputs.sh full)
J1=$(sbatch --parsable --dependency=afterok:${J0} scripts/01_preprocessing_qc.sh)
J2=$(sbatch --parsable --dependency=afterok:${J1} scripts/02_assembly.sh)
J3=$(sbatch --parsable --dependency=afterok:${J2} scripts/03_validation.sh)
J4=$(sbatch --parsable --dependency=afterok:${J3} scripts/04_mapping_dedup_filter.sh)
J5=$(sbatch --parsable --dependency=afterok:${J4} scripts/05_coverage_depth.sh)
J6=$(sbatch --parsable --dependency=afterok:${J5} scripts/06_poolseq_pipeline.sh)
J7=$(sbatch --parsable --dependency=afterok:${J6} scripts/07_qc_visualization.sh)
sbatch --dependency=afterok:${J7} scripts/08_neutral_connectivity.sh
```

---


<!-- POLOCO_COLLAPSE_END -->
</details>
<details>
<summary><strong>🧭 Neutral population structure and connectivity</strong></summary>
<!-- POLOCO_COLLAPSE_START -->
## 🧭 Neutral population structure and connectivity

Step 08 demonstrates how the allele-frequency matrix produced by PoLoCo can be used immediately for a concise population-genomic analysis. It performs:

- principal component analysis of neutral allele-frequency variation;
- spatial visualization of PC1;
- calculation of pairwise genetic and geographic distances;
- a Mantel test for isolation by distance.

The analysis uses:

```text
PoPoolation2/results/geno_AF_matrix_LD_MAF005.csv
PoPoolation2/metadata/bamlist_clean.txt
connectivity_example/plot_coordinates.csv
```

For the complete case study, Step 08 runs automatically after Step 07. For another dataset, enable it in the selected project-specific configuration and provide a matching coordinate file.

The results are written to:

```text
10_connectivity_analysis/
├── neutral_connectivity_results.xlsx
├── PCA_neutral_structure.png
├── PCA_PC1_spatial_map.png
├── isolation_by_distance.png
├── connectivity_summary.tsv
├── analysis_interpretation.txt
├── pca_scores.csv
├── pca_variance.csv
└── pairwise_distances.csv
```

The Excel workbook contains the summary statistics, PCA variance, PCA scores, aligned coordinates, and pairwise distance table. The interpretation file reports whether the Mantel test supports a statistically significant isolation-by-distance pattern.

To run only this step after Step 06 has completed:

```bash
bash scripts/run_poloco_pipeline.sh --step 08
```

---

<!-- POLOCO_COLLAPSE_END -->
</details>
<details>
<summary><strong>📊 Main outputs</strong></summary>
<!-- POLOCO_COLLAPSE_START -->
## 📊 Main outputs

The main output folders are:

```text
01_raw_reads/assembly/       raw paired-end reads for draft genome assembly
01_raw_reads/pools/          raw paired-end Pool-seq reads
02_trimmed_reads/assembly/   trimmed assembly reads
02_trimmed_reads/pools/      trimmed Pool-seq reads
02_fastqc_reports/           fastp, FastQC, and MultiQC reports
03_alignments/               mapped BAM files
04_bam_filtered/             deduplicated and mapping-quality-filtered BAM files
05_coverage/                 coverage summaries
08_assembly/                 draft assembly outputs
09_validation/               QUAST, FastANI, and alignment validation outputs
10_connectivity_analysis/       PCA, spatial connectivity, Mantel-test figures and tables
PoPoolation2/metadata/       BAM lists and Pool-seq metadata
PoPoolation2/results/        mpileup, sync file, SNP table, allele-frequency matrix
07_plots/                    QC plots and integrated QC reports
```

For the manuscript case study, the key final allele-frequency matrix is:

```text
PoPoolation2/results/geno_AF_matrix_LD_MAF005.csv
```

Other important Pool-seq outputs include:

```text
PoPoolation2/results/envilis.mpileup
PoPoolation2/results/envilis.sync
PoPoolation2/results/envilis_snps.sync
PoPoolation2/results/envilis_snps_LD_MAF005.txt
```

The exact prefix depends on `PROJECT_PREFIX` in the selected configuration. For the fixed smoke test, the corresponding outputs are written under `smoke_test/output/`.

The allele-frequency matrix currently has no header row. Column 1 is the SNP identifier, and the remaining columns follow the exact BAM order stored in:

```text
PoPoolation2/metadata/bamlist_clean.txt
```

Always archive `bamlist_clean.txt` with the allele-frequency matrix. A plain sample-order file can be generated with:

```bash
sed -E 's#.*/##; s/\.filtered\.bam$//' \
  PoPoolation2/metadata/bamlist_clean.txt \
  > PoPoolation2/metadata/sample_order.txt
```

---

<!-- POLOCO_COLLAPSE_END -->
</details>
<details>
<summary><strong>🧪 Downstream use</strong></summary>
<!-- POLOCO_COLLAPSE_START -->
## 🧪 Downstream use

PoLoCo outputs can be used for downstream analyses such as:

- PCA or ordination of allele-frequency variation,
- pairwise genetic differentiation,
- reference-comparison analyses,
- landscape-genomic or genotype-environment association analyses.

These are downstream uses of PoLoCo outputs and are not required for running the core workflow.

---

<!-- POLOCO_COLLAPSE_END -->
</details>
<details>
<summary><strong>Troubleshooting</strong></summary>
<!-- POLOCO_COLLAPSE_START -->
## Troubleshooting

### Conda is available on the login node but not inside a SLURM job

Create `configs/slurm_config.sh` from the supplied example and set the correct module or Conda initialization command:

```bash
cp configs/slurm_config.example.sh configs/slurm_config.sh
```

For example:

```bash
MODULE_SETUP='module load devel/miniforge'
```

Alternatively, set `CONDA_INIT` in `configs/slurm_config.sh` to the full path of the site's `conda.sh` file.

### Conda environment creation fails

Try updating conda first:

```bash
conda update -n base -c defaults conda
```

Then rerun:

```bash
bash configs/install_poloco_conda_envs.sh
```

### BUSCO tries to download during an HPC assembly job

Download the lineage before submitting the full workflow:

```bash
conda run -n poloco_assembly busco --download arthropoda_odb10
```

Run this from the repository root and verify that `busco_downloads/lineages/arthropoda_odb10` exists.

### PoPoolation2 is not found

Step 06 automatically detects `mpileup2sync.pl` after activating `poloco_poolseq`. Check the installation with:

```bash
conda run -n poloco_poolseq bash -lc 'command -v mpileup2sync.pl'
```

If automatic discovery is not possible, set the absolute path in the selected project-specific override:

```bash
MPILEUP2SYNC="/absolute/path/to/mpileup2sync.pl"
```

### The small ENA example fails during Pool-seq filtering

If you downloaded only a few pools using `--max-pools`, create a project-specific override and change its minimum population threshold instead of modifying the supplied case-study configuration.

For example:

```bash
MINPOP_MODE="absolute"
MINPOP_ABS=2
```

or:

```bash
MINPOP_MODE="proportion"
MINPOP_PROP=0.67
```

### The workflow cannot find paired reads

Check that files are in the expected folders and match the suffixes in the selected configuration:

```bash
READ1_SUFFIX="_R1.fastq.gz"
READ2_SUFFIX="_R2.fastq.gz"
```

### A workflow step needs to be rerun

Inspect the job state and the end of both `.out` and `.err` logs before removing anything. Do not run two copies of the workflow against the same output paths.

Step 02 skips an existing complete assembly, but stops when the assembly directory exists without the expected final contigs. Step 04 processes samples sequentially, so inspect existing BAM outputs carefully before restarting it.

Step 06 writes new mpileup and sync data to temporary files and assigns the final filenames only after successful completion. An existing mpileup is checked for valid structure and reference bases before it is reused. An existing sync must be non-empty before downstream filtering begins.

Use a separate project output root for each dataset or parameter set.


### Step 08 cannot match the coordinate file to the allele-frequency matrix

Step 08 derives the sample order from `PoPoolation2/metadata/bamlist_clean.txt`. Check that every sample has one coordinate row and that the file contains exactly:

```text
pool_id,x,y
```

For the case-study naming pattern, both `ENIV_pool_2501` and `2501` resolve to the same pool identifier. For other datasets, use the same sample names as the BAM filenames without the `.filtered.bam` suffix.

Coordinates must be projected in metres. Do not use longitude and latitude in decimal degrees for the distance analysis.

### Locale warnings in WSL

Messages such as the following can appear in WSL:

```text
warning: setlocale: LC_ALL: cannot change locale
```

These warnings do not affect PoLoCo results when the workflow continues normally and the expected output files are produced.

---
<!-- POLOCO_COLLAPSE_END -->

</details>

## Reproducibility records

For a documented analysis, retain:

- the Git commit hash;
- the main configuration and any override used;
- exact Conda exports or explicit package specifications;
- the ENA manifest or local input manifest;
- input checksums;
- SLURM job IDs, resource requests, final states, and logs;
- `PoPoolation2/metadata/bamlist_clean.txt`;
- the filtering summary;
- checksums for the final SNP table, allele-frequency matrix, and integrated QC workbook.

The fixed smoke test is intended as the first installation check. It should complete Steps 00–07 and retain eight final sites with the supplied data and thresholds.

---

## 🙌 Contributing

Contributions, bug reports, and feature requests are welcome. Please open an issue or pull request on GitHub.

---

## 📖 Citation

If you use this workflow, please cite:

> Mohammad Jamil Shuvo *et al.* (2026). **PoLoCo: A reproducible pooled low-coverage workflow for draft genome assembly and allele frequency analysis from ethanol-preserved small non-model invertebrates**. [Journal link pending].

Also cite the core tools used by the workflow:

- [fastp](https://github.com/OpenGene/fastp)
- [FastQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/)
- [MultiQC](https://multiqc.info/)
- [BWA](https://github.com/lh3/bwa)
- [samtools](http://www.htslib.org/)
- [Picard](https://broadinstitute.github.io/picard/)
- [PoPoolation2](https://sourceforge.net/projects/popoolation2/)
- [MEGAHIT](https://github.com/voutcn/megahit)
- [BUSCO](https://busco.ezlab.org)
- [QUAST](https://github.com/ablab/quast)
- [FastANI](https://github.com/ParBLiSS/FastANI)
- [MUMmer4](https://github.com/mummer4)

---

## 📜 License

This project is released under the MIT License.

