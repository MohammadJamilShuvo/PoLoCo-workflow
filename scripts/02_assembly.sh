...
ASSEMBLY_BASENAME="$(basename "${ASSEMBLY_R1}")"
ASSEMBLY_SAMPLE="${ASSEMBLY_BASENAME%${READ1_SUFFIX}}"
R1="${ASSEMBLY_TRIM_DIR}/${ASSEMBLY_SAMPLE}${TRIMMED_READ1_SUFFIX}"
R2="${ASSEMBLY_TRIM_DIR}/${ASSEMBLY_SAMPLE}${TRIMMED_READ2_SUFFIX}"
...
if [[ "${RUN_BUSCO}" == "yes" ]]; then
    ...
    echo "[SKIP] BUSCO disabled by RUN_BUSCO=${RUN_BUSCO}."
fi

echo "[OK] Assembly and reference preparation finished."
