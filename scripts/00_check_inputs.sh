...
if [[ "${MODE}" == "full" || "${MODE}" == "poolseq-only" ]]; then
    if [[ -n "${MPILEUP2SYNC:-}" && -f "${MPILEUP2SYNC}" ]]; then
        echo "[OK] PoPoolation2 mpileup2sync.pl: ${MPILEUP2SYNC}"
    elif command -v conda >/dev/null 2>&1 && conda run -n poloco_poolseq bash -lc 'command -v mpileup2sync.pl' >/dev/null 2>&1; then
        echo "[OK] PoPoolation2 mpileup2sync.pl detected in poloco_poolseq."
    else
        echo "[WARN] mpileup2sync.pl was not detected. Step 06 will check again after activating poloco_poolseq."
    fi
fi
