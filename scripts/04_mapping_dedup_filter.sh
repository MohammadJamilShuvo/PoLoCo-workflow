...
for r1 in "${POOL_TRIM_DIR}"/*"${TRIMMED_READ1_SUFFIX}"; do
    ...
    bwa mem -t "${THREADS_MAPPING}" "${REF_FASTA}" "$r1" "$r2" > "$sam"
    samtools sort -@ "${THREADS_MAPPING}" -o "$sorted_bam" "$sam"
    rm -f "$sam"
    samtools index "$sorted_bam"

    if [[ "${REMOVE_DUPLICATES}" == "yes" ]]; then
        picard MarkDuplicates I="$sorted_bam" O="$dedup_bam" M="$metrics" REMOVE_DUPLICATES=true VALIDATION_STRINGENCY=SILENT
