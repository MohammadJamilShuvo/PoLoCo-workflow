...
QC_TIME="12:00:00"
CONNECTIVITY_CPUS=2
CONNECTIVITY_MEM="16G"
CONNECTIVITY_TIME="04:00:00"
...
case "${MODE}" in
    full)
        STEPS=(00 01 02 03 04 05 06 07 08)
        ;;
    assembly-only)
        STEPS=(00 01 02 03)
        ;;
    poolseq-only)
        STEPS=(00 01 04 05 06 07 08)
        ;;
esac
...
for step in "${STEPS[@]}"; do
    case "${step}" in
        ...
        07)
            ...
            ;;
        08)
            job_name="poloco_08_connectivity"
            script="scripts/08_neutral_connectivity.sh"
            description="Neutral population structure and connectivity"
            cpus="${CONNECTIVITY_CPUS}"
            mem="${CONNECTIVITY_MEM}"
            time_limit="${CONNECTIVITY_TIME}"
            log_prefix="08_neutral_connectivity"
            script_args=()
            ;;
    esac
