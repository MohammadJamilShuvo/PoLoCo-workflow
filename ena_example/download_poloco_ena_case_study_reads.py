...
def download_file(
    ...
    if skip_existing and out_path.exists() and out_path.stat().st_size > 0:
        if verify_md5 and expected_md5 and expected_md5.lower() not in {"nan", "none", ""}:
            ...
            if observed.lower() == expected_md5.lower():
                print(f"[SKIP] Existing file with verified MD5: {out_path}")
                return
            print(f"[WARN] Existing file failed MD5 and will be downloaded again: {out_path}")
            out_path.unlink()
        else:
    ...
    for attempt in range(1, retries + 1):
        ...
        print(f"           -> {out_path} (attempt {attempt}/{retries}, resume byte {offset})")
        try:
            ...
            if attempt == retries:
                raise SystemExit(f"[ERROR] Download failed for {url} after {retries} attempts: {exc}")
            print(f"[WARN] Download interrupted: {exc}")
...
def main() -> None:
    ...
    parser.add_argument("--retries", type=int, default=3,
                        help="Number of attempts for interrupted downloads (default: 3).")
    ...
    for url, out, expected_md5, _sample in planned:
        download_file(url, out, expected_md5, skip_existing=True, verify_md5=not args.no_md5, retries=args.retries)
